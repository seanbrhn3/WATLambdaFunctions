...
#Read the file and get the sneaker name
    def scrape_shoes(self):
        sneakDB = requests.get(self.link+"/v1/sneakers?limit=100&page=5")
        the_snaeaks = sneakDB.json()
        for sneak in the_snaeaks['results']:
            self.download_google_staticimages(sneak['title'])
...
def main():
    men = WATScrper("https://api.thesneakerdatabase.com","")
    men.scrape_shoes()
