...
class WATScraper:
    ...
    def scrape_shoes(self):
        try:
            sneaker_head_image = api_call()
            # sql_client = SQLClient() RDS is fucking expensive!!!
            ...
            for sneaker in sneaker_head_image:
                ...
                # CODE ONLY USED FOR RDS:
                #sneaker["s3_prefix"] = title+"/"
                # sql_client.insert(sneaker)
                
                self.client.put_object(Bucket="sagemakertestwat-dev", Key=title)
                # Adds image to S3 directory
